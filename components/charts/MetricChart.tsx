...
function generateSyntheticCategories(socials: string[] = []) {
...
export function MetricChart({ metric, chartType, accountId, connectedSocials = [] }: MetricChartProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const dbSeries = useMetricSeriesV2(accountId, metric.key as "posts"|"comments"|"likes"|"followers");

  const timeSeries = useMemo(() => dbSeries.length ? dbSeries : generateSyntheticTimeSeries(metric.key), [dbSeries, metric.key]);
  const categories = useMemo(() => generateSyntheticCategories(connectedSocials), [connectedSocials]);

  let chartContent: React.ReactNode = null;

  switch (chartType) {
    case "line":
      chartContent = <LineChartWithStats data={timeSeries} metricLabel={metric.label} variant="line" />;
      break;
    case "area":
      chartContent = <LineChartWithStats data={timeSeries} metricLabel={metric.label} variant="area" />;
      break;
    case "bar":
    ...
    case "pie":
      ...
      break;
    case "radar":
      chartContent = <RadarChartWithStats userId={accountId} />;
      break;
  }

  return (
    <div className="space-y-4">
      <div ref={containerRef} className="w-full">{chartContent}</div>
